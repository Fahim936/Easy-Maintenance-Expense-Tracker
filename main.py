...
def filter_expenses(filter_text):
  global connector, table

  table.delete(*table.get_children())

  all_data = connector.execute(
    f'SELECT * FROM ExpenseTracker WHERE Payee LIKE "{filter_text}%"')
  data = all_data.fetchall()
...
def total_expenses_today():
  ...
  all_data = connector.execute(
    f'SELECT Amount FROM ExpenseTracker WHERE Date="{today_date}"')
  data = all_data.fetchall()

  for value in data:
    today_total += value[0]

  mb.showinfo('Total Expenses Today',
              f'Total expenses today: {today_total:.2f}')
...
def sort_expenses_custom():
    global connector, table, from_date, to_date

    table.delete(*table.get_children())

    from_dt = from_date.get_date().strftime('%Y-%m-%d')
    to_dt = to_date.get_date().strftime('%Y-%m-%d')

    all_data = connector.execute(f'SELECT * FROM ExpenseTracker WHERE Date BETWEEN "{from_dt}" AND "{to_dt}"')
    data = all_data.fetchall()
...
def edit_expense():
  ...
  edit_btn = Button(data_entry_frame,
                    text='Edit expense',
                    font=btn_font,
                    width=30,
                    bg=hlb_btn_bg,
                    command=edit_existing_expense)
  edit_btn.place(x=10, y=383)
...
root = Tk()
root.title('EMET Project')
root.geometry('1200x550')
root.resizable(0, 0)

Label(root,
      text='EASY MAINTENANCE EXPENSE TRACKER',
...
filter_btn = Button(data_entry_frame,
                    text='Filter',
                    font=btn_font,
                    width=15,
                    bg=hlb_btn_bg,
                    command=lambda: filter_expenses(filter_text.get()))
filter_btn.place(x=10, y=430)
...
dd1 = OptionMenu(
  data_entry_frame, MoP, *[
    'Cash', 'Cheque', 'Credit Card', 'Debit Card', 'Paytm', 'Google Pay',
    'Phonepe', 'Razorpay', 'Netbanking'
  ])
dd1.place(x=160, y=305)
dd1.configure(width=10, font=entry_font)

Button(data_entry_frame,
       text='Add expense',
       command=add_another_expense,
       font=btn_font,
       width=30,
       bg=hlb_btn_bg).place(x=10, y=383)
Button(data_entry_frame,
       text='Convert to words before adding',
       font=btn_font,
       width=30,
       bg=hlb_btn_bg).place(x=10, y=475)
...
Button(buttons_frame,
       ...
       width=25,
       bg=hlb_btn_bg,
       command=remove_expense).place(x=15, y=5)
...
Button(buttons_frame,
       ...
       width=25,
       bg=hlb_btn_bg,
       command=clear_fields).place(x=400, y=5)
...
Button(buttons_frame,
       ...
       width=25,
       bg=hlb_btn_bg,
       command=remove_all_expenses).place(x=660, y=5)

from_date = DateEntry(root, width=8, background='darkblue', foreground='white', borderwidth=2)
from_date.pack(padx=2 ,pady=10)

to_date = DateEntry(root, width=8, background='darkblue', foreground='white', borderwidth=2)
to_date.pack(padx=2,pady=3)

sort_custom_btn = Button(root, text='Sort Custom', command=sort_expenses_custom)
sort_custom_btn.pack(padx=5,pady=0)
...
Button(buttons_frame,
       ...
       width=25,
       bg=hlb_btn_bg,
       command=total_expenses_today).place(x=15, y=65)
...
Button(buttons_frame,
       ...
       font=btn_font,
       width=25,
       bg=hlb_btn_bg).place(x=400, y=65)
...
Button(buttons_frame,
       ...
       width=25,
       bg=hlb_btn_bg,
       command=selected_expense_to_words).place(x=660, y=65)

filter_text = StringVar()
filter_entry = Entry(data_entry_frame,
                     textvariable=filter_text,
                     width=30,
                     font=entry_font)
filter_entry.place(x=160, y=435)
# Treeview Frame
